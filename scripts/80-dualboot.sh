...
if ! command -v grub-mkconfig &>/dev/null || [ ! -f "/etc/default/grub" ]; then
    warn "GRUB not detected. Skipping dual-boot configuration."
    exit 0
fi
...
section "Optional 80" "Dual-Boot Configuration (Windows)"
...
if [ "$WINDOWS_DETECTED" != "true" ]; then
    ...
    log "Module 80 completed (Skipped)."
    exit 0
fi
